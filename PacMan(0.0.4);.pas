...
//процедура отрисовки игрового пол€, представл€ющего собой массив 20х20 клеток
procedure Pole;
var
  i, j: integer;
begin
  ...
  begin
    for j := 0 to 19 do
    begin
      a[i, j] := 0; //заполнение массива 0, которые обозначают поле по которому пакмен может двигатьс€
    end;
  end;
  for i := 0 to 19 do
  begin
    ...
    begin
      ...
      SetPenColor(clBlack);
      if a[i, j] = 0 then rectangle(i * 20, j * 20, (i * 20) + 20, (j * 20) + 20); //если эл-нт массива равен 0, то рисуетс€ квадрат     
    end;
  end;
...
begin
  ...
  n2p := n2;
  m2p := m2;
  // выбор движени€ первого призрака по координате x или y
  l := random(2) + 1;
  if l = 1 then 
  begin
  ...
  setbrushcolor(clPink);  //ќтрисовка розового призрака
  circle(n2, m2, 10); 
  ...
  setbrushcolor(clRed); //ќтрисовка красного призрака
  circle(n, m, 10); 
...
//процедура нажати€ кнопки Settings
